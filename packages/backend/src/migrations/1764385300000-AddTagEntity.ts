...
export class AddTagEntity1764385300000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "note_tags" ADD CONSTRAINT "FK_note_tags_noteId" FOREIGN KEY ("noteId") REFERENCES "notes"("id") ON DELETE CASCADE ON UPDATE CASCADE`,
    );
    await queryRunner.query(
      `ALTER TABLE "note_tags" ADD CONSTRAINT "FK_note_tags_tagId" FOREIGN KEY ("tagId") REFERENCES "tags"("id") ON DELETE CASCADE ON UPDATE CASCADE`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "note_tags" DROP CONSTRAINT "FK_note_tags_tagId"`,
    );
    await queryRunner.query(
      `ALTER TABLE "note_tags" DROP CONSTRAINT "FK_note_tags_noteId"`,
    );
